<div>
    ...
    <div class="kpi-grid mb-5" style="grid-template-columns: repeat(3, 1fr);">
        <x-hex.kpi-card label="Total Invoiced" :value="\App\Support\Inr::format($totalInvoiced)" />
        <x-hex.kpi-card label="Total Received" :value="\App\Support\Inr::format($totalReceived)" />
        <x-hex.kpi-card label="Receivables" :value="\App\Support\Inr::format($totalOutstanding)" />
    </div>
        ...
        <div class="table-scroll">
            @if ($sales->count())
                <table class="data-table">
                    <thead><tr><th>Date</th><th>Customer</th><th>Unit</th><th class="num">Invoiced</th><th class="num">Received</th><th class="num">Balance</th><th>Notes</th>@can('create', App\Models\Sale::class)<th class="num">Actions</th>@endcan</tr></thead>
                    <tbody>
                        @foreach ($sales as $sale)
                            <tr wire:key="sa-{{ $sale->id }}">
                                <td>{{ $sale->txn_date?->format('d M Y') ?? $sale->as_of_date?->format('d M Y') ?? '—' }}</td>
                                <td>{{ $sale->customer_name }}</td>
                                <td><x-hex.tag :unit="$sale->costCenter->name" /></td>
                                <td class="num amt"><x-hex.money :amount="$sale->total_invoiced" /></td>
                                <td class="num amt rec"><x-hex.money :amount="$sale->total_received" /></td>
                                <td class="num amt pay"><x-hex.money :amount="$sale->balance" /></td>
    ...
    @if ($showForm)
        <div class="modal-overlay show"><div class="modal">
            ...
            <form wire:submit="save" class="modal-body"><div class="form-grid">
                ...
                <label class="span-2"><span>Customer</span><input type="text" wire:model="formCustomer"></label>
